...
# Now blast
blastn -task blastn -db $reference_geno -query for_blast.fa -outfmt "6 std qseq sseq slen" -num_threads $num_threads -out blast_out.txt
...
if [ $(find ./KASP_output/ -name "selected_KASP_primers*" 2>/dev/null | wc -l) -gt 0 ]; then
    # Step 1: Concatenate
    python3 "$script_dir/bin/concat_KASP_SPP_output.py" "./KASP_output" "Potential_KASP_primers.tsv" "selected_KASP_primers" 20
    ...
    # Step 4: blast and add that to output
    python3 "$script_dir/bin/blast_primer_analysis.py" $reference_geno tmp_blast_output.tsv > blast_primer_analysis.py.log
